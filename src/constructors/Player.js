...
class Player {
  ...
  roll = function (direction, healthBars, shields, dashIcons, soundOn) {
    if (this.rollCount > 0) {
      ...
      createjs.Tween.get(healthBars[0], { override: true }).to(
        { x: healthBars[0].x + value },
        500
      );

      createjs.Tween.get(shields[0], { override: true }).to(
        { x: shields[0].x + value },
        500
      );

      createjs.Tween.get(dashIcons[0], { override: true }).to(
        { x: dashIcons[0].x + value },
        500
      );

      createjs.Tween.get(dashIcons[1], { override: true }).to(
        { x: dashIcons[1].x + value },
        500
      );

      createjs.Tween.get(dashIcons[2], { override: true }).to(
        { x: dashIcons[2].x + value },
        500
      );
