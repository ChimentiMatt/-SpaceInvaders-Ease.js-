...
class Beam {
  ...
  moveBeams = function (player, soundOn) {
    ...
    if (this.sprite.y === player.y - 17) {
      createjs.Tween.get(this.sprite).to({ y: -16 }, 1500);
    }
  };
  ...
  detectCollision = function (beams, enemies) {
    ...
    for (let i = 0; i < enemies.length; i++) {
      ...
      if (enemies[i].sprite.currentAnimation !== "dead") {
        ...
        if (
          this.sprite.y <= enemies[i].sprite.y + spriteHeight &&
          this.sprite.y >= enemies[i].sprite.y - spriteHeight
        ) {
          ...
          if (this.sprite.currentAnimation !== "contact") {
            ...
            if (
              this.sprite.x >= enemies[i].sprite.x - 8 &&
              this.sprite.x <= enemies[i].sprite.x + spriteWidth
            ) {
              if (!this.madeContact) {
